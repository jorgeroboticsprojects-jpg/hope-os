...
%% Set sofas
%                                posX, posY, posZ, width X, long Y, height Z.
occMatrix = placeBlock(occMatrix, 10, 25, 1, 15, 50, 10); %big sofa bottom
occMatrix = placeBlock(occMatrix, 10, 25, 11, 5, 50, 10); %big sofa back support

occMatrix = placeBlock(occMatrix, 10, 25, 1, 50, 15, 10); %big sofa bottom
occMatrix = placeBlock(occMatrix, 10, 25, 11, 50, 5, 10); %big sofa back support

% occMatrix = placeBlock(occMatrix, 66, 59, 1, 15, 15, 10); %small sofa bottom
% occMatrix = placeBlock(occMatrix, 76, 59, 11, 5, 15, 10); %small sofa back support
% 
% occMatrix = placeBlock(occMatrix, 66, 29, 1, 15, 15, 10); %small sofa bottom
% occMatrix = placeBlock(occMatrix, 76, 29, 11, 5, 15, 10); %small sofa back support
% 
% occMatrix = placeBlock(occMatrix, 42, 34, 1, 15, 30, 8); %small sofa bottom
...
desks = [
      36 71 0 1 0;  % Desk at [30,55], horizontal, with monitor
...
chairs = [
%      44 75 0;    % Chair just behind the desk
%     %68 66 90
];
% 
% %% Format: [x, y, rotation, width, height, levels, depth]
shelves = [
        71 75  0  20  20  2 16;   % Left shelf
        83 7  90  40 50  5 8;   % Right shelf
%      1 20 270  20 20  1 15;   % Small Left shelf
];
% 
% %% Format: [x, y, height, radius]
trashBins = [
%     10 10 16 5   % Trash bin in lower-right corner
];
% 
...
tableH = 11;   % height of table surface
legH = 10;     % height of table legs
...
for i = 1:size(desks,1)
    ...
    if mon == 1
        if rot == 0
            mx = px + round(deskL/2) - 14;
            if monSide == 0
                my = py + deskW - 8;  % default front
            else
                my = py + 5;          % opposite side
            end
            occMatrix = placeBlock(occMatrix, mx, my, tableH+5, 28, 1, 15);    % screen
            occMatrix = placeBlock(occMatrix, mx+12, my, tableH+1, 4, 1, 9);    % stand
        else
...
save('../../Maps_ROI_Object_Situations/LivingRoom_2_situations/LivingRoom_2OccMatrix.mat','occMatrix');
disp('saved as LivingRoom_2OccMatrix.mat ')
% 
% %% Save WorkSpace
save('../../Maps_ROI_Object_Situations/LivingRoom_2_situations/LivingRoom_2WorkSpace.mat','WorkSpace');
disp('saved as LivingRoom_2WorkSpace.mat ')
