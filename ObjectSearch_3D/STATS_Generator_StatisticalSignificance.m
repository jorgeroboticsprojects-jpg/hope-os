...
fileNames = {'Test_results_data/All_7_Maps_booster_100_2.mat'}%, 'Test_results_data/All_7_Maps_booster_100_3.mat'}; % Updated with joined file
playerNames = {'HOPE-OS 3D'}%, 'test3'};
analyze3DExploration(fileNames, playerNames);
...
nexttile;
boxplot(padDataForBoxplot(finalDistances));
title('Total Travelled Distance');
ylabel('Distance [meters]');
...
% ======= HELPERS =======
function T = makeStatsAndPrint(dataMat, playerNames, metricName)
    % Calculates avrg/std per column (groupo/legend)
    mu = mean(dataMat, 1, 'omitnan');        % 1xG
    sd = std(dataMat, 0, 1, 'omitnan');      % 1xG
    T  = table(playerNames(:), mu(:), sd(:), ...
...
function cumulativeAngDist = computeCumulativeAngularDistance(positions)
    ...
    cumulativeAngDist = zeros(1, size(positions, 1));
    
    for i = 2:size(positions, 1)
        % Extract quaternions at steps i-1 and i
        q1 = positions(i-1, 4:7); % Previous orientation
        q2 = positions(i, 4:7);   % Current orientation

        % Compute the relative rotation: deltaQ = q2 * conjugate(q1)
        deltaQ = quatmultiply(q2, quatconj(q1));

        % Ensure the quaternion is normalized
        deltaQ = deltaQ / norm(deltaQ);

        % Extract the rotation angle from the scalar part (qw = cos(theta/2))
        angle = 2 * acos(min(1, max(-1, deltaQ(1))));  % Clamp value to [-1, 1] for numerical safety

        % Accumulate the angular distance
